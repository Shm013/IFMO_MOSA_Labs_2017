...
VOID RtlTypeFile(WCHAR *filename)
{
	HANDLE hFile = NULL;
	BYTE Buf[8192];
	LONGLONG fileSize = 0;
	...
	bResult = NtFileOpenFile(&hFile, filename, FALSE, FALSE);
	if (bResult == FALSE) 
	{
		//return FALSE;
		return;
	...
	if (NtFileGetFileSize(hFile, &fileSize) == FALSE) 
	{
	...
	while (1) 
	{
		...
		if (NtFileReadFile(hFile, Buf, 8192, &readedSize) == FALSE) 
		{
			NtFileCloseFile(hFile);
			//return FALSE;
			return;
		}
		RtlCliDisplayString(Buf);
...
NTSTATUS __cdecl main(INT argc, PCHAR argv[], PCHAR envp[], ULONG DebugFlag OPTIONAL)
{
    ...
    // Show banner
    //
	RtlCliDisplayString("Windows must die!\nType 'quit' to exit\n");
